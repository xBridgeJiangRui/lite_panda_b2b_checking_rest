...
class Api extends REST_Controller{
    ...
    {
        $customer_guid = $this->db->query("SELECT * FROM lite_b2b.acc WHERE acc_doc_name = 'TF Value Mart' LIMIT 1");
            ...
            foreach($refno->result() as $row)
            {
                ...
                if(count($gr_child->result()) > 0)
                {
                    foreach($gr_child->result() as $row2)
                    {
                        ...
                        {
                            $this->db->query("UPDATE backend.pomain SET send = 2 WHERE RefNo = '$row2->PORefNo'");
                        }
                        else
                        {
                            $this->db->query("UPDATE backend.pomain SET send = 999 WHERE RefNo = '$row2->PORefNo'");
